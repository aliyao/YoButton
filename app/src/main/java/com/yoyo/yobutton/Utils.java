 ...
 * 类描述：
 * 创建人：yoyo
 * 创建时间：2016/9/28 10:12
 * 修改人：yoyo
 * 修改时间：2016/9/28 10:12
 * 修改备注：
 */

public class Utils {
    /**
     * 对 windowManager 进行设置
     *
     */
   /* public static WindowManager.LayoutParams getParams(WindowManager.LayoutParams wmParams) {
        wmParams = new WindowManager.LayoutParams();
        // 设置 window type 下面变量 2002 是在屏幕区域显示，2003 则可以显示在状态栏之上
     ...
     * 1 个，因为有可能最近任务中包好 Launcher2。 这样可以保证我们展示出来的 最近任务 为 MAX_RECENT_TASKS 个
     * 通过以下步骤，可以获得近期任务列表，并将其存放在了 appInfos 这个 list 中，接下来就是展示这个 list 的工作了。
     */
    public static void reloadButtons(Service service, List<HashMap<String, Object>> appInfos, int appNumber) {
        ...
        // 得到包管理器和 activity 管理器
        final Context context = service.getApplication();
        final PackageManager pm = context.getPackageManager();
        final ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);

        // 从 ActivityManager 中取出用户最近 launch 过的 MAX_RECENT_TASKS + 1 个，以从早到晚的时间排序，
        // 注意这个 0x0002, 它的值在 launcher 中是用 ActivityManager.RECENT_IGNORE_UNAVAILABLE
        ...
        for (int i = 0; i < numTasks && (i < MAX_RECENT_TASKS); i++) {
            ...
            /**
             * 如果找到是 launcher，直接 continue，后面的 appInfos.add 操作就不会发生了
             */
            if (homeInfo != null) {
    ...
    public static void virtualHome(Context mContext) {
        ...
        i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK); // 如果是服务里调用，必须加入 new task 标识
    ...
    public static void virtualBack(AccessibilityService service) {
        if (Build.VERSION.SDK_INT < 16) {
            Toast.makeText(service, "Android 4.1 及以上系统才支持此功能，请升级后重试 ",  Toast.LENGTH_LONG).show();
        } else {
